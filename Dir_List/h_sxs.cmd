...
if not exist %WorkSystemRoot%\WinSxS (echo error: %WorkSystemRoot%\WinSxS not found &pause &goto :eof)

ECHO.
ECHO Program Path = %ProgDir%
ECHO WinSxS  Path = %WorkSystemRoot%\WinSxS
ECHO.
echo Search for Single and Multi Hardlink data in WinSxS  Path - can take 10 minutes ....
echo.
...
echo.
echo Search for Single and Multi Hardlink WinSxS data is Running ..... Please Wait - can take 10 minutes ....
echo.
...
rem for all nested files in all folders in WinSxS sorted by name ....
rem make filelist Single_Hard_Link_List_%datum%.txt of all files in WinSxS that refer to single hardlink data

for /f %%e in ('dir %WorkSystemRoot%\WinSxS /B /AD-L /ON') do (
  set /A folder_cnt=!folder_cnt!+1
  set fol_name=%%e
  if /I NOT !fol_name!==Backup (
  if /I NOT !fol_name!==Catalogs (
  if /I NOT !fol_name!==FileMaps (
    for /f %%f in ('dir %WorkSystemRoot%\WinSxS\%%e /B /S /A-D-L /ON') do (
      ...
      if !mod_rest! EQU 0 (
        echo Total folder count = !folder_cnt!
